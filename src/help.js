const help = (prefix) => {
	return `li.⌜ ATX by 에이퀴즈 ⌟.il

☆ *INFORMACION*
   ⭑ Cᵒᵐᵃᶰᵈᵒ: ⌜ ${prefix} ⌟
   ⭑ Aᵖᵖ: https://App.ATX@is.gd/VrtzXT
   ⭑ Cʳᵉᵃᵈᵒʳ: ATX 
   ⭑ https://ATX-Main@is.gd/5yk050
   ⭑ *PARA USAR EL BOT* Rᵉᵍᶤˢᵗʳᵃᵗᵉ ᶜᵒᶰ ᵉˡ ᶜᵒᵐᵃᶰᵈᵒ ${prefix}ᵈᵃᶠᵗᵃʳ ʸ ᵗᵘ ᶰᵒᵐᵇʳᵉ


☆ *DUDAS*
  ⎡
  ⎢ ⭑ ${prefix}Vᵉʳˢᶤᵒᶰ (Cᵒᶰᵒᶜᵉ ˡᵃ ᵛᵉʳˢᶤᵒᶰ ᵈᵉ ᵗᵘ ᵇᵒᵗ)
  ⎣ ⭑ ${prefix}ᶜʳᵉᵃᵈᵒʳ (Dᵘᵈᵃˢ ᵒ ᵖʳᵒᵇˡᵉᵐᵃˢ ᵃᵠᵘᶤ)
  
  ☆ *ATX CASE*
  ⎡
  ⎢ ⭑ ${prefix}ATX
  ⎣ ⭑ ${prefix}UES
 
☆ *CREAR STICKERS*
  ⎡
  ⎢ ⭑ ${prefix}ˢᵗᶤᶜᵏᵉʳ
  ⎢ ⭑ ${prefix}ᵃᵗᵗᵖ (Mᵃˢ ᵘᶰ ᵗᵉˣᵗᵒ ᶜᵒʳᵗᵒ)
  ⎣ ⭑ ${prefix}ˢᵗᶤᶜᵏᵉʳᵍᶤᶠ (6 ˢᵉᵍᵘᶰᵈᵒˢ ᵈᵉ ᵛᶤᵈᵉᵒ)

☆ *CONVERTIDORES*
  ⎡
  ⎢ ⭑ ${prefix}ᵈᵉˢᵐᵉᶰᵘ (Dᵉˢᶜᵃʳᵍᵃʳ ᵐᵘˢᶤᶜᵃ ʸ ᵛᶤᵈᵉᵒˢ ᵈᵉ YT)
  ⎢ ⭑ ${prefix}ᵗᵒᶤᵐᵍ (Dᵉ ˢᵗᶤᶜᵏᵉʳ ᵃ JPG)
  ⎣ ⭑ ${prefix}ᵗᵒᵐᵖ3 (Dᵉ ᵛᶤᵈᵉᵒ ᵃ MP3)

☆ *AUDIO*
  ⎡
  ⎢ ⭑ ${prefix}ᶤᵈᶤᵒᵐᵃ
  ⎣ ⭑ ${prefix}ᵗᵗˢ ᵉˢ (ᵐᵃˢ ᵗᵉˣᵗᵒ)

☆ *OTROS*
  ⎡
  ⎢ ⭑ ${prefix}ʷᵃᵐᵉ (Lᶤᶰᵏ ᵈᵉ Wʰᵃᵗˢᵃᵖᵖ)
  ⎢ ⭑ ${prefix}ᵠʳᶜᵒᵈᵉ (Cᵒˡᵒᶜᵃ ᵘᶰ ᵗᵉˣᵗᵒ)
  ⎢ ⭑ ${prefix}ᵒᵗᵃᵏ (Mᵒᶰᵃˢ ᶜʰᶤᶰᵃˢ)
  ⎢ ⭑ ${prefix}ˢʰᵃᶰᵗᵉʳᵃ (Iᶰᵗᵉʳᵃᶜᵗᵘᵃ ᶜᵒᶰ ᵉˡ ᵇᵒᵗ)) 
  ⎣ ⭑ ${prefix}ʲᵘᵉᵍᵒˢ (Dᶤᵛᶤᵉʳᵗᵉ ᶜᵒᶰ ᵗᵘˢ ᵃᵐᶤᵍᵒˢ :))

☆ *GRUPOS*
  ⎡
  ⎢ ⭑ ${prefix}ʷᵉˡᵐᵉᶰᵘ (Cᵒᵐᵃᶰᵈᵒˢ ᵈᵉ ᵇᶤᵉᶰᵛᵉᵈᶤᵃ ᵃ ᵍʳᵘᵖᵒˢ)
  ⎢ ⭑ ${prefix}ᵃᶰᵗᶤᵐᵉᶰᵘ (Sᵘ ᶰᵒᵐᵇʳᵉ ˡᵒ ᵈᶤᶜᵉ ᵗᵒᵈᵒ (ᵃᶰᵗᶤˡᶤᶰᵏ))
  ⎢ ⭑ ${prefix}ᶜˡᵒˢᵉᵍᶜ (Cᵉʳʳᵃʳ ᵉˡ ᵍʳᵘᵖᵒ ˢᵒˡᵒ ᵃᵈᵐᶤᶰˢ)
  ⎢ ⭑ ${prefix}ᵒᵖᵉᶰᵍᶜ (Aᵇʳᶤʳ ᵍʳᵘᵖᵒ ˢᵒˡᵒ ᵃᵈᵐᶤᶰˢ)
  ⎢ ⭑ ${prefix}ᵏᶤᶜᵏ ᵒ ᵖᵃᶠᵘᵉʳᵃ (Eˡᶤᵐᶤᶰᵃʳ ᵃ ᵘᶰ ᵐᶤᵉᵐᵇʳᵒ) 
  ⎢ ⭑ ${prefix}ᵖʳᵒᵐᵒᵗᵉ (Dᵃʳ ᵃᵈᵐᶤᶰ ᵃ ᵘᶰ ᵐᶤᵉᵐᵇʳᵒ)
  ⎢ ⭑ ${prefix}ᵈᵉᵐᵒᵗᵉ (Qᵘᶤᵗᵃʳ ᵉˡ ᵃᵈᵐᶤᶰ)
  ⎢ ⭑ ${prefix}ˡᶤᶰᵏᵍᶜ (Lᶤᶰᵏ ᵈᵉˡ ᵍʳᵘᵖᵒ)
  ⎢ ⭑ ${prefix}ˡᶤˢᵗᵃᵈᵐᶤᶰˢ (Nᵒᵐᵇʳᵃʳ ᵃ ˡᵒˢ ᵃᵈᵐᶤᶰ)
  ⎣ ⭑ ${prefix}ᵗᵒᵈᵒˢ (Nᵒᵐᵇʳᵃ ᵃ ᵗᵒᵈᵒˢ ˡᵒˢ ᵈᵉˡ ᵍʳᵘᵖᵒ)


ву ATX

li.⌜ 에이퀴즈 ⌟.il 
`
}

exports.help = help
